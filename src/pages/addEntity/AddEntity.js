...
const AddEntity = (props) => {
  ...
  const handleNameCheck = (e, entityId) => {
    let inputValue = e.target.value;
    inputValue = inputValue.trim();
    let vendorList = [];
    //In case of edit entity removing the entity to be edited
    if (entityId) {
      vendorList = props.vendorList.filter((eachVendor) => {
        return eachVendor.entityId != entityId;
      });
    } else {
      vendorList = props.vendorList;
    }
    if (vendorList) {
      const result = vendorList.some((v) => {
        const entityShortName = v.shortName;
        return entityShortName === inputValue;
      });
  ...
  const handleLongNameCheck = (e, entityId) => {
    let inputValue = e.target.value;
    inputValue = inputValue.trim();
    let vendorList = [];
    ...
    if (vendorList) {
      const result = vendorList.some((v) => {
        const entityLongName = v.longName;
        return entityLongName === inputValue;
      });
  ...
  const steps = [
    {
      ...
      content: (
        <NewVendorForm
          ...
          handleLongNameCheck={handleLongNameCheck}
        />
      ),
    },
    {
  ...
  const next = () => {
    if (!shortNameFound && !longNameFound) formRef.current.submit();
  };
  useEffect(() => {
    if (validForm) {
  ...
  const handleSubmitSuccess = async () => {
    ...
    let roleNameUpdated = localStorage.getItem("entitlementType");
    const formData = {
      ...formValues,
      ...(!params.id && {
        createdBy: userNameUpdated,
        roleName: roleNameUpdated,
      }),
      entityUpdateFlag: params.id ? "Y" : "N",
      ...(params.id && {
        entityId: params.id,
    ...
    const res = params.id
      ? await startUpdateEntity(formData)
      : await dispatch(startAddVendor(formData));
